...
$isOffline = FALSE;
if (Config('offline') == 'offline') $isOffline = TRUE;
else {
	$offlineTimeSpansConfig =Config('offline_times');
	if (strlen($offlineTimeSpansConfig)) {
		...
		foreach ($timeSpans as $timeSpan) {
			...
			if ($fromTimestamp <= $now && $now <= $toTimestamp) {
				...
				break;}}}}
if ($isOffline) {
	$parameters['offline_message'] = nl2br(Config('offline_text'));
	...
	if (!isset($_SESSION['badgechecked']) && $website->getUser()->getRole() == ROLE_USER && $website->getUser()->getClubId($website, $db)) {
		...
		$result = $db->querySelect('datum_anmeldung',Config('db_prefix') . '_user', 'id = %d', $userId);
