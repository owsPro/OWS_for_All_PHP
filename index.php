...
*****************************************************************************/
								...
								$userId);$userinfo=$result->fetch_array();if($userinfo['datum_anmeldung']){$numberOfRegisteredDays=round((Timestamp()-$userinfo['datum_anmeldung'])/(3600*24));awardBadgeIfApplicable($website,$db,$userId,
								'membership_since_x_days',$numberOfRegisteredDays);}$_SESSION['badgechecked']=1;}$pageId=Request('page');$pageId=getTargetPageId($website,$i18n,$pageId);setPageId($pageId);$validationMessages=null;$actionId=Request('action');
								if($actionId!==NULL){try{$targetId=handleAction($website,$db,$i18n,$actionId);if($targetId!=null)$pageId=$targetId;}catch(ValidationException$ve){$validationMessages=$ve->getMessages();
								...
								$e->getMessage()));}}setPageId($pageId);$navItems=getNavigationItems($website,$i18n,$page,$pageId);$parameters['navItems']=$navItems;$parameters['breadcrumbItems']=getBreadcrumbItems($website,$i18n,$page,
