...
*****************************************************************************/
								...
								$errors['inputEmail']=Message('sendpassword_admin_usernotfound');elseif($admin['passwort_neu_angefordert']>($now-120*60))$errors['inputEmail']=Message('sendpassword_admin_alreadysent');else{$newPassword=SecurityUtil::generatePassword();
								$hashedPw=SecurityUtil::hashPassword($newPassword,$admin['passwort_salt']);$columns=array('passwort_neu'=>$hashedPw,'passwort_neu_angefordert'=>$now);$fromTable=$conf['db_prefix'].'_admin';$whereCondition='id=%d';$parameter=$admin['id'];
								$db->queryUpdate($columns,$fromTable,$whereCondition,$parameter);try{sendEmail($inputEmail,$newPassword,$website,$i18n);header('location:login.php?newpwd=1');die();}catch(Exception$e){$errors['inputEmail']=$e->getMessage();}}
								$result->free();}}?><!DOCTYPE html><html><head><title>AdminCenter - <?php echo Message('sendpassword_admin_title');?></title><meta charset='UTF-8'><link rel='shortcut icon'type='image/x-icon'href='../favicon.ico'/><style type='text/css'>
