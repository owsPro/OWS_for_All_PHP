...
*****************************************************************************/
	include($_SERVER['DOCUMENT_ROOT'].'/admin/config/global.inc.php');$i18n=I18n::getInstance(Config('supported_languages'));if(isset($_GET['lang']))$i18n->setCurrentLanguage($_GET['lang']);include(sprintf(escapeOutput(CONFIGCACHE_ADMINMESSAGES,
	$i18n->getCurrentLanguage())));include(sprintf(escapeOutput($_SERVER['DOCUMENT_ROOT']).'/languages/messages_%s.php',$i18n->getCurrentLanguage()));$errors=[];$inputEmail=(isset($_POST['inputEmail']))?trim($_POST['inputEmail']):FALSE;if($inputEmail)
