...
*****************************************************************************/
					...
					$now=Timestamp();foreach($items as$item){echo'<tr>';$jobid=(string)$item->getAttribute('id');$i18nJobNameAttr='name_'.$i18n->getCurrentLanguage();if($item->hasAttribute($i18nJobNameAttr))$name=
					...
					if($lastPing)echo FormattedDatetime($lastPing);else echo'-';echo'</td>';echo'<td>'.$interval.' '.Message('unit_minutes').'</td>';echo'<td>'.$status.'</td>';echo'<td>';if($running)echo"<a href=\"job.php?action=stop&id=".$jobid.
