...
?>
    ...
    <form action='<?php echo escapeOutput($_SERVER['PHP_SELF']);?>' method='post' class='form-horizontal'>
        ...
        <fieldset>
            ...
            $formFields['lastlogindays']=['type'=>'number','value'=>(isset($_POST['lastlogindays']))?escapeOutput($_POST['lastlogindays']):60];
            $formFields['maxplayers']=['type'=>'number','value'=>(isset($_POST['maxplayers']))?escapeOutput($_POST['maxplayers']):15];
            $formFields['userid']=['type'=>'foreign_key','value'=>(isset($_POST['userid']))?escapeOutput($_POST['userid']):'','entity'=>'users','jointable'=>'user','labelcolumns'=>'nick'];
            foreach($formFields as $fieldId=>$fieldInfo)
                echo createFormGroup($i18n, $fieldId, $fieldInfo, $fieldInfo['value'], 'firemanagers_search_');
            ?>
        </fieldset>
...
if(!empty($_POST['maxbudget']) || !empty($_POST['lastlogindays']) || !empty($_POST['maxplayers']) || !empty($_POST['userid'])){
    $columns=[];
    $columns=['C.id'=>'team_id','C.name'=>'team_name','C.finanz_budget'=>'team_budget','(SELECT COUNT(*)FROM '.Config('db_prefix').'_spieler AS PlayerTab WHERE PlayerTab.verein_id=C.id AND status=\'1\')'=>'team_players','U.id'=>'user_id','U.nick'=>'user_nick','U.lastonline'=>'user_lastonline'];
    $fromTable=Config('db_prefix').'_verein AS C INNER JOIN '.Config('db_prefix').'_user AS U ON U.id=C.user_id';
    ...
    if(!empty($_POST['lastlogindays'])){
        ...
        $parameters[]=Timestamp()-$_POST['lastlogindays']*24*3600;
    }
        ...
        <form action='<?php echo escapeOutput($_SERVER['PHP_SELF']);?>' method='post' class='form-horizontal' id='frmMain' name='frmMain'>
            ...
            <table class='table table-striped table-hover'>
                ...
                <tbody>
                    ...
                    while($row=$result->fetch_array()){
                        echo '<tr>
                            <td><input type=\'checkbox\' name=\'selectedteams[]\' value=\''.$row['team_id'].'\'/></td>
                            <td class=\'tableRowSelectionCell\'><a href=\''.iUrl('team','id='.$row['team_id']).'\' target=\'_blank\'>'.escapeOutput($row['team_name']).'</a></td>
                            <td class=\'tableRowSelectionCell\'><a href=\''.iUrl('user','id='.$row['user_id']).'\' target=\'_blank\'>'.escapeOutput($row['user_nick']).'</a></td>
                            <td class=\'tableRowSelectionCell\'>'.FormattedDate($row['user_lastonline']).'</td>
                            <td class=\'tableRowSelectionCell\'>'.number_format($row['team_budget'],0,',',' ').'</td>
                            <td class=\'tableRowSelectionCell\'>'.$row['team_players'].'</td>
                        </tr>'.PHP_EOL;
                    }
                    ?>
                </tbody>
            ...
            <p>
                <label class='checkbox'>
                    <input type='checkbox' name='selAll' value='1' onClick='selectAll()'><?php echo Message('manage_select_all_label');?>
                </label>
            </p>
            <p>
                <input type='submit' class='btn btn-primary' accesskey='l' title='Alt + l' value='<?php echo Message('firemanagers_dismiss_button');?>'>
            </p>
        </form>
        <?php 
    }
    ...
    if(!isset($_POST['selectedteams']) || !count($_POST['selectedteams'])){
        ...
        <form action='<?php echo escapeOutput($_SERVER['PHP_SELF']);?>' method='post' class='form-horizontal'>
            ...
            <fieldset>
                ...
                <?php 
                $formFields=[];
                $formFields['disableusers']=['type'=>'boolean','value'=>0];
                $formFields['minbudget']=['type'=>'number','value'=>1000000];
                $formFields['message_subject']=['type'=>'text','value'=>''];
                $formFields['message_content']=['type'=>'textarea','value'=>''];
                foreach($formFields as $fieldId=>$fieldInfo)
                    echo createFormGroup($i18n, $fieldId, $fieldInfo, $fieldInfo['value'], 'firemanagers_dismiss_');
                ?>
            </fieldset>
            <fieldset>
                ...
                <?php 
                $formFields=[];
                $formFields['mincontractmatches']=['type'=>'number','value'=>15];
                $formFields['strength_min_freshness']=['type'=>'percent','value'=>70];
                $formFields['strength_min_stamina']=['type'=>'percent','value'=>70];
                $formFields['strength_min_satisfaction']=['type'=>'percent','value'=>70];
                foreach($formFields as $fieldId=>$fieldInfo)
                    echo createFormGroup($i18n, $fieldId, $fieldInfo, $fieldInfo['value'], 'firemanagers_dismiss_');
                ?>
            </fieldset>
            <fieldset>
                ...
                <?php 
                $formFields=[];
                $formFields['option_T']=['type'=>'number','value'=>2];
                $formFields['option_LV']=['type'=>'number','value'=>1];
                $formFields['option_IV']=['type'=>'number','value'=>2];
                $formFields['option_RV']=['type'=>'number','value'=>1];
                $formFields['option_LM']=['type'=>'number','value'=>1];
                $formFields['option_DM']=['type'=>'number','value'=>1];
                $formFields['option_ZM']=['type'=>'number','value'=>1];
                $formFields['option_OM']=['type'=>'number','value'=>1];
                $formFields['option_RM']=['type'=>'number','value'=>1];
                $formFields['option_LS']=['type'=>'number','value'=>1];
                $formFields['option_MS']=['type'=>'number','value'=>1];
                $formFields['option_RS']=['type'=>'number','value'=>1];
                $formFields['player_age']=['type'=>'number','value'=>25];
                $formFields['player_age_deviation']=['type'=>'number','value'=>3];
                $formFields['entity_player_vertrag_gehalt']=['type'=>'number','value'=>10000];
                $formFields['entity_player_vertrag_spiele']=['type'=>'number','value'=>60];
                $formFields['entity_player_w_staerke']=['type'=>'percent','value'=>50];
                $formFields['entity_player_w_technik']=['type'=>'percent','value'=>50];
                $formFields['entity_player_w_kondition']=['type'=>'percent','value'=>70];
                $formFields['entity_player_w_frische']=['type'=>'percent','value'=>80];
                $formFields['entity_player_w_zufriedenheit']=['type'=>'percent','value'=>80];
                $formFields['playersgenerator_label_deviation']=['type'=>'percent','value'=>10];
                foreach($formFields as $fieldId=>$fieldInfo)
...
elseif($show=='dismiss'){
    if($admin['r_demo'])
        $err[]=Message('validationerror_no_changes_as_demo');
    if(isset($err))
        ...
        foreach($teamIds as $teamId){
            ...
            if(!empty($_POST['message_subject']) && !empty($_POST['message_content'])){
                $db->queryInsert([
                    ...
                    'absender_name'=>Config('projectname'),
                    'absender_id'=>'',
                    'datum'=>Timestamp(),
            ...
            $result=$db->querySelect('id,position_main,w_kondition,w_frische,w_zufriedenheit,vertrag_spiele',Config('db_prefix').'_spieler','verein_id=%d AND status=\'1\'',$teamId);
            while($player=$result->fetch_array()){
                ...
                $db->queryUpdate(['w_frische'=>$freshness,'w_kondition'=>$stamina,'w_zufriedenheit'=>$satisfaction,'vertrag_spiele'=>$contractMatches],Config('db_prefix').'_spieler','id=%d',$player['id']);
            ...
            if($playersToGenerate)
                generatePlayers($website,$db,$teamId,$_POST['player_age'],$_POST['player_age_deviation'],$_POST['entity_player_vertrag_gehalt'],$_POST['entity_player_vertrag_spiele'],$strengths,$positions,$_POST['playersgenerator_label_deviation']);
        }
