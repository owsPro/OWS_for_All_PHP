...
*****************************************************************************/
					...
					'active_home'=>$homeActive,'minute'=>$minute,'goals'=>$goals,'playernames'=>$playerNames],Config('db_prefix').'_matchreport');}}echo'<form action=\''.escapeOutput($_SERVER['PHP_SELF']).'\'class=\'form-horizontal\'method=\'post\'>
					...
					escapeOutput($match['match_home_name']).'</option><option value=\''.$match['match_guest_id'].'\'>'.escapeOutput($match['match_guest_name']).'</option></select></div></div>';echo createFormGroup($i18n,'message_id',
					...
					$homeTeam=escapeOutput($match['match_home_name']);$guestTeam=escapeOutput($match['match_guest_name']);foreach($reportItems as$reportItem){echo'<tr><td><a href=\'?site=$site&action=delete&match=$matchId&itemid='.escapeOutput($reportItem['report_id']).
	  				...
	  				echo Message('option_'.escapeOutput($reportItem['type']));echo'</td><td>'.escapeOutput($reportItem['message']).'</td><td>'.escapeOutput($reportItem['playerNames']).'</td><td>'.escape($reportItem['goals']).'</td></tr>';}echo'</tbody></table>';}