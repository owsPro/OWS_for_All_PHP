...
*****************************************************************************/
					...
					'active_home'=>$homeActive,'minute'=>$minute,'goals'=>$goals,'playernames'=>$playerNames],Config('db_prefix').'_matchreport');}}echo'<form action=\''.ESC($_SERVER['PHP_SELF']).'\'class=\'form-horizontal\'method=\'post\'>
					...
					ESC($match['match_home_name']).'</option><option value=\''.$match['match_guest_id'].'\'>'.ESC($match['match_guest_name']).'</option></select></div></div>';echo createFormGroup($i18n,'message_id',
					['type'=>'foreign_key','jointable'=>'spiel_text','entity'=>'matchtext','labelcolumns'=>'aktion,nachricht'],'','match_manage_reportmsg_');echo createFormGroup($i18n,'playernames',['type'=>'text'],'','match_manage_reportmsg_');
					echo createFormGroup($i18n,'minute',['type'=>'number'],'','match_manage_reportmsg_');echo createFormGroup($i18n,'intermediateresult',['type'=>'text'],'','match_manage_reportmsg_');echo'</fieldset>
					...
					$homeTeam=ESC($match['match_home_name']);$guestTeam=ESC($match['match_guest_name']);foreach($reportItems as$reportItem){echo'<tr><td><a href=\'?site=$site&action=delete&match=$matchId&itemid='.ESC($reportItem['report_id']).
	  				'\'title=\''.Message('manage_delete').'\'class=\'deleteLink\'><i class=\'icon-trash\'></i></a> '.ESC($reportItem['minute']).'</td><td><small>';if($reportItem['active_home'])echo$homeTeam;else echo$guestTeam;echo'</small><br>';
	  				echo Message('option_'.ESC($reportItem['type']));echo'</td><td>'.ESC($reportItem['message']).'</td><td>'.ESC($reportItem['playerNames']).'</td><td>'.escape($reportItem['goals']).'</td></tr>';}echo'</tbody></table>';}
