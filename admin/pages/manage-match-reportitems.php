...
*****************************************************************************/
					...
					$homeTeam=escapeOutput($match['match_home_name']);$guestTeam=escapeOutput($match['match_guest_name']);foreach($reportItems as$reportItem){echo'<tr><td><a href=\'?site=$site&action=delete&match=$matchId&itemid='.$reportItem['report_id'].
	  				'\'title=\''.Message('manage_delete').'\'class=\'deleteLink\'><i class=\'icon-trash\'></i></a> '.$reportItem['minute'].'</td><td><small>';if($reportItem['active_home'])echo$homeTeam;else echo$guestTeam;echo'</small><br>';echo Message('option_'.
					$reportItem['type']);echo'</td><td>'.$reportItem['message'].'</td><td>'.$reportItem['playerNames'].'</td><td>'.$reportItem['goals'].'</td></tr>';}echo'</tbody></table>';}
