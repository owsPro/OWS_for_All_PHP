...
if (!$show) { ?>
	...
	if ($leagueid > 0) {
	  $columns = ["T1.id" => "id", "T1.name" => "name", "(SELECT COUNT(*) FROM " . $conf['db_prefix'] . "_spieler AS S WHERE S.verein_id = T1.id)" => "playerscount"];
	    ...
	    <table class="table table-striped">
	    	<thead>
	    		<tr>
	    			...
	    			<th><?php echo $i18n->getMessage("playersgenerator_head_playerscount"); ?></th></tr></thead>
	    	<tbody><?php
...
elseif ($show == "generateform") { ?>
  <form action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post" class="form-horizontal">
	...
	$formFields = ["player_age" => ["type" => "number", "value" => 25],"player_age_deviation" => ["type" => "number", "value" => 3],"entity_player_vertrag_gehalt" => ["type" => "number", "value" => 10000],
		"entity_player_vertrag_spiele" => ["type" => "number", "value" => 60],"entity_player_w_staerke" => ["type" => "percent", "value" => 50],"entity_player_w_technik" => ["type" => "percent", "value" => 50],
		"entity_player_w_kondition" => ["type" => "percent", "value" => 70],"entity_player_w_frische" => ["type" => "percent", "value" => 80],"entity_player_w_zufriedenheit" => ["type" => "percent", "value" => 80],
...
elseif ($show == "generate") {
  ...
  if (isset($err)) include("validationerror.inc.php");
	...
	$positions = ["T" => $_POST["option_T"],"LV" => $_POST["option_LV"],"IV" => $_POST["option_IV"],"RV" => $_POST["option_RV"],"LM" => $_POST["option_LM"],"ZM" => $_POST["option_ZM"],"RM" => $_POST["option_RM"],"DM" => $_POST["option_DM"],"OM" => $_POST["option_OM"],
		"LS" => $_POST["option_LS"],"MS" => $_POST["option_MS"],"RS" => $_POST["option_RS"]];
