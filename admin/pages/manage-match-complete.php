...
*****************************************************************************/
							...
							$observer->onMatchCompleted($matchModel);echo createSuccessMessage(Message('match_manage_complete_success'),'');}echo"<h3><a href=\"".iUrl('team','id='.$match['match_home_id'])."\"target=\'_blank\'>".ESC($match['match_home_name']).
							"</a> - <a href=\"".iUrl('team','id='.$match['match_guest_id'])."\"target=\'_blank\'>".ESC($match['match_guest_name'])."</a></h3><div class=\"well\">".Message('match_manage_complete_intro')."</div><form action=\"".ESC($_SERVER['PHP_SELF']).
