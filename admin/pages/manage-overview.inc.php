...
if($deleteEnabled&&$action=="delete"){
	if(isset($id)&&$id)$del_id=array($id);if($admin['r_demo'])throw new Exception($i18n->getMessage('error_access_denied'));if(count($del_id)){$dependencies=ModuleConfigHelper::findDependentEntities($dbTableWithoutPrefix);foreach($del_id as$deleteId){if($loggingEnabled){
	$result=$db->querySelect($loggingColumns,$dbTable,"id=%d",$deleteId);$item=$result->fetch_array(MYSQLI_ASSOC);$result->free();logAdminAction($website,LOG_TYPE_DELETE,$admin['name'],$entity,json_encode($item));}$db->queryDelete($dbTable,'id=%d',$deleteId);
	foreach($dependencies as$dependency){$fromTable=Config('db_prefix').'_'.$dependency['dbtable'];$whereCondition=$dependency['columnid'].'=%d';if(strtolower($dependency['cascade'])=='delete')$db->queryDelete($fromTable,$whereCondition,$deleteId);
	else$db->queryUpdate(array($dependency['columnid']=>0),$fromTable,$whereCondition,$deleteId);}}echo createSuccessMessage($i18n->getMessage('manage_success_delete'),'');}}
	...
	foreach($outputColumns as $fieldId => $columnInfo) {
		...
		if ($fieldId == "entity_". $entity ."_status")echo " style=\"width: 20px;\">&nbsp;";
			...
			if ($columnInfo["sort"]) {
				...
				if($sortColumn==$fieldId)$header.=" <a href=\"".UrlUtil::buildCurrentUrlWithParameters(array("site"=>$site,"entity"=>$entity,PARAM_RESETSORT=>1))."\" title=\"". $i18n->getMessage("manage_sort_column_reset") ."\"><i class=\"icon-remove-sign\"></i></a>";}
			echo ">". $header;}
		echo "</th>";}
