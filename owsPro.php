...
class Value{static$pageId;}
			...
			function getFormattedDate($timestamp){return date(Config('date_format'),$timestamp);}
...
class WebSoccer{private static$_instance;private$_skin,$_pageId,$_templateEngine,$_frontMessages,$_user,$_contextParameters;
			...
			function getFormattedDatetime($timestamp,$i18n=null){if($timestamp==null)$timestamp=Timestamp();if($i18n!=null){$dateWord=StringUtil::convertTimestampToWord($timestamp,$tTimestamp(),$i18n);if(strlen($dateWord))
...
class ActionHandler{
	 static function handleAction($website,$db,$i18n,$actionId){if($actionId==NULL)return;if(isset($_SESSION['laction_id'])&&$_SESSION['laction_id']==$actionId&&isset($_SESSION['laction_time'])&&($_SESSION['laction_time']+config('DOUBLE_SUBMIT_CHECK_SECONDS'))>
							Timestamp()){throw new Exception(Message('error_double_submit'));}$actionConfig=json_decode($website->getAction($actionId),true);$actionXml=ModuleConfigHelper::findModuleConfigAsXmlObject($actionConfig['module']);
...
class FormBuilder {
	static function createFormGroup($i18n,$fieldId,$fieldInfo,$fieldValue,$labelKeyPrefix){
		...
		if(hasMessage($inlineHelpKey))$helpText='<span class=\'help-inline\'>'.Message($inlineHelpKey).'</span>';
		if($type=='boolean'){
			...
			switch($type){
				...
				case 'timestamp':
					$website=WebSoccer::getInstance();
					$dateFormat=Config('date_format');
					if(!$fieldValue)$fieldValue=Timestamp();
				...
				case 'select':
					...
					foreach($selection as$selectItem){
						...
						echo '>';
						$label=$selectItem;
						if(hasMessage('option_'.$selectItem))$label=Message('option_'.$selectItem);
...
class YouthMatchSimulationExecutor {
	static function simulateOpenYouthMatches($websoccer,$db,$maxMatchesToSimulate){
		...
		$result=$db->querySelect('*',Config('db_prefix').'_youthmatch', 'simulated!=\'1\' AND matchdate <= %d ORDER BY matchdate ASC',Timestamp(),$maxMatchesToSimulate);
		while($matchinfo=$result->fetch_array()){
...
class StadiumEnvironmentPlugin {
	...
	static function getBonusSumFromBuildings($websoccer,$db,$attributeName,$teamId){
		$dbPrefix=Config('db_prefix');
		$result=$db->querySelect('SUM('.$attributeName.')AS attrSum',$dbPrefix.'_buildings_of_team INNER JOIN '.$dbPrefix.'_stadiumbuilding ON id=building_id', 'team_id=%d AND construction_deadline<%d',array($teamId,Timestamp()));
...
class RssResultsOfUserModel extends Model {
	function getTemplateParameters(){
		...
		$items=[];
		foreach($matches as$match)$items[]=array('url'=>iUrl('match', 'id='.$match['id'],TRUE), 'title'=>$match['home_team'].' - '.$match['guest_team'].' ('.$match['home_goals'].':'.$match['guest_goals'].')',
...
class DirectTransfersDataService {
	static function createTransferOffer($websoccer,$db,$playerId,$senderUserId,$senderClubId,$receiverUserId,$receiverClubId,$offerAmount,$offerMessage,$offerPlayerId1=null,$offerPlayerId2=null){
		$columns=array("player_id"=>$playerId,"sender_user_id"=>$senderUserId,"sender_club_id"=>$senderClubId,"receiver_club_id"=>$receiverClubId,"submitted_date"=>Timestamp(),"offer_amount"=>$offerAmount,
	...
	static function _transferPlayer($websoccer,$db,$playerId,$targetClubId,$targetUserId,$currentUserId,$currentClubId,$amount,$exchangePlayer1=0,$exchangePlayer2=0){
		...
		$db->queryInsert(array("bid_id"=> 0,"datum"=>Timestamp(),"spieler_id"=>$playerId,"seller_user_id"=>$currentUserId,"seller_club_id"=>$currentClubId,"buyer_user_id"=>$targetUserId,"buyer_club_id"=>$targetClubId,
...
function renderRound($roundNode){global$i18n,$website,$hierarchy,$site,$cupid,$cup,$action,$db;
	...
	elseif($action == "edit-save" && $_REQUEST["id"] == $roundNode["round"]["id"]) {
		...
		$firstDateObj = DateTime::createFromFormat(Config("date_format") .", H:i",$_POST["firstround_date_date"] .", ". $_POST["firstround_date_time"]);
		$columns["firstround_date"] = $firstDateObj->getTimestamp();
		if (isset($_POST["secondround_date_date"])) {
			$secondDateObj = DateTime::createFromFormat(Config("date_format") .", H:i",
				$_POST["secondround_date_date"] .", ". $_POST["secondround_date_time"]);
			$columns["secondround_date"] = $secondDateObj->getTimestamp();
		}
		$db->queryUpdate($columns,Config("db_prefix") . "_cup_round", "id = %d", $roundNode["round"]["id"]);
		if ($roundNode["round"]["name"] !== $_POST["name"]) {
			$db->queryUpdate(array("pokalrunde" => $_POST["name"]),Config("db_prefix") . "_spiel", "pokalname = '%s' AND pokalrunde = '%s'",
					array($cup["name"], $roundNode["round"]["name"]));
		}
		$result = $db->querySelect("*",Config("db_prefix") . "_cup_round", "id = %d", $roundNode["round"]["id"]);
	...
	if($showEditForm){?>
		...
		<div class="control-group">
			<div class="controls">
				...
				<a href="<?php echo "?site=" . escapeOutput($site) . "&cup=" . escapeOutput($cupid); ?>" class="btn"><?php echo Message("button_cancel"); ?></a>
			</div>
		</div>
	  </form>
		<?php }
		...
		echo "</strong>";
		echo " <a href=\"?site=". escapeOutput($site) . "&cup=". escapeOutput($cupid) . "&action=edit&id=". $roundNode["round"]["id"] . "\" title=\"". Message("manage_edit") . "\"><i class=\"icon-pencil\"></i></a>";
		echo " <a class=\"deleteLink\" href=\"?site=". escapeOutput($site) . "&cup=". escapeOutput($cupid) . "&action=delete&id=". $roundNode["round"]["id"] . "\" title=\"". Message("manage_delete") . "\"><i class=\"icon-trash\"></i></a>";
		echo "</p>";
		echo "<ul>";
		echo "<li><em>" . Message("managecuprounds_label_firstround_date")  . ":</em> ". date(FormattedDatetime($roundNode["round"]["firstround_date"])) . "</li>";
		if ($roundNode["round"]["secondround_date"]) {
			echo "<li><em>" . Message("managecuprounds_label_secondround_date")  . ":</em> ". date(FormattedDatetime($roundNode["round"]["secondround_date"])) . "</li>";
		}
