...
if(\R('contentonly'))echo(htmlentities($output['content']);																				// Check if the 'contentonly' parameter is present in the request
    ...
    echo \json_encode(htmlentities($output)),JSON_THROW_ON_ERROR|JSON_UNESCAPED_UNICODE);}												// JSON_THROW_ON_ERROR throws an exception if an error occurs during JSON encoding, and JSON_UNESCAPED_UNICODE ensures that Unicode characters are not escaped.