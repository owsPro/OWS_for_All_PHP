...
class BreadcrumbBuilder{
	 static function getBreadcrumbItems($website,$i18n,$pages,$currentPageId){
							if(!isset($pages[$currentPageId]))return;$items=[];$nextPageId=$currentPageId;while($nextPageId){$pageConfig=json_decode($pages[$nextPageId],TRUE);$items[$nextPageId]=$i18n->getNavigationLabel($nextPageId);
							if(isset($pageConfig['parentItem'])&& strlen($pageConfig['parentItem']))$nextPageId=$pageConfig['parentItem'];else$nextPageId=FALSE;}return array_reverse($items);}}
class ConfigCacheFileWriter{private$_frontCacheFileWriter,$_adminCacheFileWriter,$_supportedLanguages,$_messagesFileWriters,$_adminMessagesFileWriters,$_entityMessagesFileWriters,$_settingsCacheFileWriter,$_eventsCacheFileWriter,$_newSettings;
			...
			function _buildModulesConfig(){$modules=scandir($_SERVER['DOCUMENT_ROOT'].'/modules');foreach($modules as$module){if(is_dir($_SERVER['DOCUMENT_ROOT'].'/modules'.'/'.$module)){$files=scandir($_SERVER['DOCUMENT_ROOT'].'/modules'.'/'.$module);
							...
							elseif(StringUtil::startsWith($file,'messages_'))$this->_processMessages($pathToFile,$this->_messagesFileWriters);elseif(StringUtil::startsWith($file,'adminmessages_'))$this->_processMessages($pathToFile,$this->_adminMessagesFileWriters);
...
class DataUpdateSimulatorObserver{private$_teamsWithSoonEndingContracts;
			...
			function onBeforeMatchStarts(SimulationMatch $match){if((Config('sim_income_trough_friendly')||$match->type!=='Freundschaft')&&!$match->isAtForeignStadium)SimulationAudienceCalculator::computeAndSaveAudience($this->_websoccer,$this->_db,$match);$clubTable=
			...
			function deductSalary(SimulationTeam $team,$salary){BankAccountDataService::debitAmount($this->_websoccer,$this->_db,$team->id,$salary,'match_salarypayment_subject','match_salarypayment_sender');}
			...
			function creditSponsorPayments(SimulationTeam$team,$isHomeTeam,$teamIsWinner){$columns='S.name AS sponsor_name,b_spiel,b_heimzuschlag,b_sieg,T.sponsor_spiele AS sponsor_matches';$fromTable=Config('db_prefix').'_verein AS T INNER JOIN '.Config('db_prefix').
							...
							$amount=$sponsor['b_spiel'];if($isHomeTeam)$amount+=$sponsor['b_heimzuschlag'];if($teamIsWinner)$amount+=$sponsor['b_sieg'];BankAccountDataService::creditAmount($this->_websoccer,$this->_db,$team->id,$amount,'match_sponsorpayment_subject',
			...
			function updateUsers(SimulationMatch$match){$highscoreWin=Config('highscore_win');$highscoreLoss=Config('highscore_loss');$highscoreDraw=Config('highscore_draw');$columns='U.id AS u_id,U.highscore AS highscore,U.fanbeliebtheit AS popularity';
							...
							BadgesDataService::awardBadgeIfApplicable($this->_websoccer,$this->_db,$homeUser['u_id'],'win_with_x_goals_difference',$goalsDiff);}elseif($match->homeTeam->getGoals()==$match->guestTeam->getGoals()){$homeColumns['highscore']=max(0,
							...
							BadgesDataService::awardBadgeIfApplicable($this->_websoccer,$this->_db,$guestUser['u_id'],'win_with_x_goals_difference',$goalsDiff);}elseif($match->guestTeam->getGoals()==$match->homeTeam->getGoals()){$popFactor=1.0;
...
class DbSessionManager{
			...
			function read($sessionId){$columns='expires,session_data';$fromTable=Config('db_prefix').'_session';$whereCondition='session_id=\'%s\'';$data='';$result=$this->_db->querySelect($columns,$fromTable,$whereCondition,$sessionId);if($result->num_rows){
					$row=$result->fetch_array();if($row['expires']<$this->_websoccer->getNowAsTimestamp())$this->destroy($sessionId);else{$data=$row['session_data'];if($data==null)$data='';}}return$data;}
			function validate_sid($key){$columns='expires';$fromTable=Config('db_prefix').'_session';$whereCondition='session_id=\'%s\'';$result=$this->_db->querySelect($columns,$fromTable,$whereCondition,$key);if($result->num_rows){$row=$result->fetch_array();
					if($row['expires']<$this->_websoccer->getNowAsTimestamp())$this->destroy($key);else{return true;}}return FALSE;}
			function write($sessionId,$data){$lifetime=(int)Config('session_lifetime');$expiry=$this->_websoccer->getNowAsTimestamp()+$lifetime;$fromTable=Config('db_prefix').'_session';$columns['session_data']=$data;$columns['expires']=$expiry;
			...
			function _deleteExpiredSessions(){$fromTable=Config('db_prefix').'_session';$whereCondition='expires<%d';$this->_db->queryDelete($fromTable,$whereCondition,$this->_websoccer->getNowAsTimestamp());}}
...
class DefaultSimulationStrategy{private$_websoccer,$_passTargetProbPerPosition,$_opponentPositions,$_shootStrengthPerPosition,$_shootProbPerPosition,$_observers;
	...
	function tackle(SimulationMatch $match){
		...
		if($result==TRUE){
			...
			if($tackled=='yellow'||$tackled=='red'){
				...
				$blockedMatches=0;
				if($injured){
					$maxMatchesInjured=(int)Config('sim_maxmatches_injured');
					$pInjuredMatches[1]=5;
	...
	function _getDefensiveStrength(SimulationTeam $team,$match){
		...
		$strength=$strength*$offensiveFactor;
		return$strength;}
			function _shootPenalty(SimulationMatch$match,SimulationPlayer$player){$goaly=SimulationHelper::selectPlayer(SimulationHelper::getOpponentTeam($player,$match),config('PLAYER_POSITION_GOALY'),null);$goalyInfluence=(int)Config('sim_goaly_influence');
					$shootReduction=round($goaly->getTotalStrength($this->_websoccer,$match)*$goalyInfluence/100);$pGoal[TRUE]=max(30,min($player->strength-$shootReduction,80));$pGoal[FALSE]=100-$pGoal[TRUE];$result=SimulationHelper::selectItemFromProbabilities($pGoal);
...
class FormBuilder {
	static function createFormGroup($i18n,$fieldId,$fieldInfo,$fieldValue,$labelKeyPrefix){
		...
		if($i18n->hasMessage($inlineHelpKey))$helpText='<span class=\'help-inline\'>'.Message($inlineHelpKey).'</span>';
		if($type=='boolean'){
	...
	static function validateField($i18n,$fieldId,$fieldInfo,$fieldValue,$labelKeyPrefix){
		...
		if(!$isEmpty){
			...
			if($fieldInfo['type']=='date'){
				...
				if(!DateTime::createFromFormat($format,$fieldValue))throw new Exception(sprintf(Message('validationerror_date'),Message($labelKeyPrefix .$fieldId),$format));}}
		if(isset($fieldInfo['validator'])&& strlen($fieldInfo['validator'])){
			$website=WebSoccer::getInstance();
			$validator=new $fieldInfo['validator']($i18n,$website,$fieldValue);
			if(!$validator->isValid())throw new Exception(Message($labelKeyPrefix .$fieldId).': '.$validator->getMessage());}}
...
class MatchSimulationExecutor {
	static function simulateOpenMatches($websoccer,$db){
		...
		$fromTable=Config('db_prefix').'_spiel AS M INNER JOIN ' .Config('db_prefix').'_verein AS HOME_T ON HOME_T.id=M.home_verein INNER JOIN ' .Config('db_prefix').
			'_verein AS GUEST_T ON GUEST_T.id=M.gast_verein LEFT JOIN ' .Config('db_prefix').'_aufstellung AS HOME_F ON HOME_F.match_id=M.id AND HOME_F.verein_id=M.home_verein LEFT JOIN ' .Config('db_prefix').
		...
		$interval=(int)Config('sim_interval');
		$maxMatches=(int)Config('sim_max_matches_per_run');
	...
	static function handleBothTeamsHaveNoFormation($websoccer,$db,$homeTeam,$guestTeam, SimulationMatch $match){
		...
		if(Config('sim_noformation_bothteams')=='computer'){
	...
	static function handleOneTeamHasNoFormation($websoccer,$db,$team, SimulationMatch $match){
		...
		if(Config('sim_createformation_without_manager')&& self::teamHasNoManager($websoccer,$db,$team->id))SimulationFormationHelper::generateNewFormationForTeam($websoccer,$db,$team,$match->id);
		else{
			if(Config('sim_noformation_oneteam')=='0_0')$match->isCompleted=TRUE;
			elseif(Config('sim_noformation_oneteam')=='3_0'){
	...
	static function createInitialMatchData($websoccer,$db,$matchinfo){
		$db->queryDelete(Config('db_prefix').'_spiel_berechnung', 'spiel_id=%d',$matchinfo['match_id']);
		$db->queryDelete(Config('db_prefix').'_matchreport', 'match_id=%d',$matchinfo['match_id']);
		$homeOffensive=($matchinfo['home_formation_offensive'])? $matchinfo['home_formation_offensive'] : Config('sim_createformation_without_manager_offensive');
		$guestOffensive=($matchinfo['guest_formation_offensive'])? $matchinfo['guest_formation_offensive'] : Config('sim_createformation_without_manager_offensive');
	...
	static function addPlayers($websoccer,$db, SimulationTeam $team,$matchinfo,$columnPrefix){
		...
		for($playerNo=1; $playerNo <= 11; $playerNo++){
			...
			if(isset($playerinfo['team_id'])&& $playerinfo['team_id']==$team->id||$team->isNationalTeam && $playerinfo['nation']==$team->name){
				...
				if($playerinfo['position']!=$position && $playerinfo['mainPosition']!=$mainPosition && $playerinfo['secondPosition']!=$mainPosition)$strength=round($strength*(1 - Config('sim_strength_reduction_wrongposition')/ 100));
		...
		if($addedPlayers<11 &&Config('sim_createformation_on_invalidsubmission')){
			$db->queryDelete(Config('db_prefix').'_spiel_berechnung', 'spiel_id=%d AND team_id=%d',array($matchinfo['match_id'],$team->id));
			$team->positionsAndPlayers=[];
...
class NavigationBuilder {
	...
	static function _createItem(&$items, &$addedItemsCache,$pageId,$pageJson,$website,$i18n,$currentPageId, &$pages){
		...
		if(isset($pageConfig['navitemOnlyForConfigEnabled'])&&!Config($pageConfig['navitemOnlyForConfigEnabled']))return;
...
class SessionBasedUserAuthentication {
	...
	function _login($userdata,$db,$fromTable,$currentUser){
		...
		$columns=array('lastonline'=>$this->_website->getNowAsTimestamp(), 'lastaction'=>Request('page'));
		$whereCondition='id=%d';
...
class SimulationAudienceCalculator {
	static function computeAndSaveAudience($websoccer,$db, SimulationMatch $match){
		...
		$maintenanceInfluence=$homeInfo['level_videowall'] *Config('stadium_videowall_effect');
		$maintenanceInfluenceSeats=(5 - $homeInfo['level_seatsquality'])*Config('stadium_seatsquality_effect');
		$maintenanceInfluenceVip=(5 - $homeInfo['level_vipquality'])*Config('stadium_vipquality_effect');
		...
		$event=new TicketsComputedEvent($websoccer,$db, I18n::getInstance(Config('supported_languages')),$match,$homeInfo['stadium_id'],$rateStands,$rateSeats,$rateStandsGrand,$rateSeatsGrand,$rateVip);
	...
	static function getHomeInfo($websoccer,$db,$teamId){
		$fromTable=Config('db_prefix').'_verein AS T INNER JOIN '.Config('db_prefix').'_stadion AS S ON S.id=T.stadion_id INNER JOIN '.Config('db_prefix').'_liga AS L ON L.id=T.liga_id LEFT JOIN '.Config('db_prefix').'_user AS U ON U.id=T.user_id';
...
class SimulationCupMatchHelper {
	static function checkIfExtensionIsRequired($websoccer,$db, SimulationMatch $match){
		...
		$result=$db->querySelect($columns,Config('db_prefix').'_spiel',$whereCondition,array($match->guestTeam->id,$match->homeTeam->id,$match->cupName,$match->cupRoundName), 1);
	...
	static function createNextRoundMatchAndPayAwards($websoccer,$db,$winnerTeamId,$loserTeamId,$cupName,$cupRound){
		...
		$result=$db->querySelect('user_id',Config('db_prefix').'_verein', 'id=%d',$winnerTeamId);
		$winnerclub=$result->fetch_array();
		$result=$db->querySelect('user_id',Config('db_prefix').'_verein', 'id=%d',$loserTeamId);
		...
		if($loserclub['user_id'])$db->queryInsert(array('user_id'=>$loserclub['user_id'],'team_id'=>$loserTeamId, 'cup_round_id'=>$round['round_id'],'date_recorded'=>$now),Config('db_prefix').'_achievement');
		if($round['is_finalround']){
			...
			$db->queryUpdate(array('winner_id'=>$winnerTeamId),Config('db_prefix').'_cup', 'id=%d',$round['cup_id']);
			...
			$fromTable=Config('db_prefix').'_cup_round';
	...
	static function checkIfMatchIsLastMatchOfGroupRoundAndCreateFollowingMatches($websoccer,$db, SimulationMatch $match){
		...
		$result=$db->querySelect('COUNT(*)AS hits',Config('db_prefix').'_spiel', 'berechnet=\'0\' AND pokalname=\'%s\' AND pokalrunde=\'%s\' AND id!=%d',array($match->cupName,$match->cupRoundName,$match->id));
		...
		$fromTable=Config('db_prefix').'_cup_round_group_next AS N INNER JOIN '.Config('db_prefix').'_cup_round AS R ON N.cup_round_id=R.id INNER JOIN '.Config('db_prefix').'_cup AS C ON R.cup_id=C.id';
	...
	static function createMatchForTeamAndRound($websoccer,$db,$teamId,$roundId,$firstRoundDate,$secondRoundDate,$cupName,$cupRound){
		$pendingTable=Config('db_prefix').'_cup_round_pending';
	...
	static function checkAndExecuteSubstitutions(SimulationMatch $match, SimulationTeam $team, $observers) {
		...
		foreach($substitutions as$substitution) {
			if($substitution->minute ==$match->minute && !isset($team->removedPlayers[$substitution->playerOut->id]) && isset($team->playersOnBench[$substitution->playerIn->id])) {
				...
				if($position != $substitution->playerIn->position) $strength = round($strength * (1 -Config("sim_strength_reduction_wrongposition") / 100));
				elseif($mainPosition != NULL && $mainPosition != $substitution->playerIn->mainPosition) $strength = round($strength * (1 -Config("sim_strength_reduction_secondary") / 100));
...
class StringUtil {
	...
	static function convertTimestampToWord($timestamp,$nowAsTimestamp,$i18n){
		...
		if($timestamp >= strtotime('tomorrow',$nowAsTimestamp))return Message('date_tomorrow');
...
class YouthMatchSimulationExecutor {
	static function simulateOpenYouthMatches($websoccer,$db,$maxMatchesToSimulate){
		if(!Config('youth_enabled'))return;
		$simulator=new Simulator($db,$websoccer);
		...
		$result=$db->querySelect('*',Config('db_prefix').'_youthmatch', 'simulated!=\'1\' AND matchdate <= %d ORDER BY matchdate ASC',$websoccer->getNowAsTimestamp(),$maxMatchesToSimulate);
		while($matchinfo=$result->fetch_array()){
	...
	static function _addPlayers($websoccer,$db, SimulationMatch $match, SimulationTeam $team){
		$fromTable=Config('db_prefix').'_youthmatch_player AS MP INNER JOIN '.Config('db_prefix').'_youthplayer AS P ON P.id=MP.player_id';
	...
	static function _createRandomFormation($websoccer,$db, SimulationMatch $match, SimulationTeam $team){
		$db->queryDelete(Config('db_prefix').'_youthmatch_player', 'match_id=%d AND team_id=%d',array($match->id,$team->id));
...
class AcceptStadiumConstructionWorkController extends Controller {
	function executeAction($parameters){
		...
		if($construction==NULL||$construction["deadline"]>$this->_websoccer->getNowAsTimestamp())throw new Exception(Message("stadium_acceptconstruction_err_nonedue"));
...
class CreateYouthMatchRequestController extends Controller {
	function executeAction($parameters){
		...
		$tooLateBoundary=$this->_websoccer->getNowAsTimestamp()+ 3600*24*(1 +Config("youth_matchrequest_max_futuredays"));
...
class DirectTransferOfferController extends Controller {
	...
	function checkPlayersTransferStop($playerId){
		...
		$transferBoundary=$this->_websoccer->getNowAsTimestamp()- 24*3600 *Config("transferoffers_transfer_stop_days");
...
class DirectTransferRejectController extends Controller {
	function executeAction($parameters){
		...
		$offer=$result->fetch_array();
		if(!$offer)throw new Exception(Message("transferoffers_offer_cancellation_notfound"));
		$this->_db->queryUpdate(array( "rejected_date"=>$this->_websoccer->getNowAsTimestamp(),"rejected_message"=>$parameters["comment"],"rejected_allow_alternative"=>($parameters["allow_alternative"])? 1 : 0),Config("db_prefix")."_transfer_offer","id=%d",$offer["id"]);
		$player=PlayersDataService::getPlayerById($this->_websoccer,$this->_db,$offer["player_id"]);
			...
			while(!feof($fp)){
				...
				if(strcmp($res,"VERIFIED")==0){
					if(strtolower($parameters["receiver_email"])!= strtolower(Config("paypal_receiver_email"))){
						EmailHelper::sendSystemEmail($this->_websoccer,Config("systememail"),"Failed PayPal confirmation: Invalid Receiver","Invalid receiver: ".$parameters["receiver_email"]);
						throw new Exception("Receiver E-Mail not correct.");}
					if($parameters["payment_status"]!="Completed"){
						EmailHelper::sendSystemEmail($this->_websoccer,Config("systememail"),"Failed PayPal confirmation: Invalid Status","A paypment notification has been sent, but has an invalid status: ".$parameters["payment_status"]);
...
class RegisterFormController extends Controller {
	...
	function _createUser($parameters,$fromTable){
		...
		EmailHelper::sendSystemEmailFromTemplate($this->_websoccer,$this->_i18n,$dbcolumns["email"],Message("activation_email_subject"),"useractivation",$tplparameters);
...
class SendPasswordController extends Controller {
	...
	function _sendEmail($email,$password){
		...
		EmailHelper::sendSystemEmailFromTemplate($this->_websoccer,$this->_i18n,$email,Message("sendpassword_email_subject"),"sendpassword",$tplparameters);}}
...
class SofortComPaymentNotificationController extends Controller {
	function executeAction($parameters){
		...
		if($SofortLibTransactionData->isError()){
			EmailHelper::sendSystemEmail($this->_websoccer,Config("systememail"),"Failed Sofort.com payment notification","Error: ".$SofortLibTransactionData->getError());
			throw new Exception($SofortLibTransactionData->getError());}
		else{
			if($SofortLibTransactionData->getStatus()!= 'received'){
				EmailHelper::sendSystemEmail($this->_websoccer,Config("systememail"),"Failed Sofort.com payment notification: invalid status","Status: ".$SofortLibTransactionData->getStatus());
...
class StadiumEnvironmentPlugin {
	...
	static function addYouthPlayerSkillBonus(YouthPlayerScoutedEvent $event){
		...
		if($bonus!=0){
			$playerTable=Config('db_prefix').'_youthplayer';
	...
	static function creditAndDebitAfterHomeMatch(MatchCompletedEvent $event){
		...
		if($sum)BankAccountDataService::creditAmount($event->websoccer,$event->db,$homeTeamId,$sum, 'stadiumenvironment_matchincome_subject',Config('projectname'));
		else BankAccountDataService::debitAmount($event->websoccer,$event->db,$homeTeamId, abs($sum), 'stadiumenvironment_costs_per_match_subject',Config('projectname'));}
...
class AbsenceModel extends Model {
	function getTemplateParameters(){
		...
		if($absence && $absence['deputy_id']){
			$result=$this->_db->querySelect('nick',Config('db_prefix').'_user', 'id=%d',$absence['deputy_id']);
...
class BadgesModel extends Model{
	function getTemplateParameters(){
		$result=$this->_db->querySelect('*',Config('db_prefix').'_badge', '1 ORDER BY event ASC, level ASC');
		$badges=[];
...
class CupGroupDetailsModel extends Model{
	function getTemplateParameters(){
		$cupRoundId=Request("roundid");
		$cupGroup=Request("group");
...
class CupResultsModel extends Model {
	function getTemplateParameters(){
		...
		$fromTable=Config('db_prefix').'_cup_round AS R INNER JOIN '.Config('db_prefix').'_cup AS C ON C.id=R.cup_id LEFT JOIN ' .Config('db_prefix').
			'_cup_round AS PREVWINNERS ON PREVWINNERS.id=R.from_winners_round_id LEFT JOIN '.Config('db_prefix').'_cup_round AS PREVLOOSERS ON PREVLOOSERS.id=R.from_loosers_round_id';
		...
		if($round['is_groupround']){
			...
			$result=$this->_db->querySelect('name,team_id',Config('db_prefix').'_cup_round_group', 'cup_round_id=%d ORDER BY name ASC',array($round['round_id']));
...
class DirectTransferOfferModel extends Model {
	function getTemplateParameters(){
		$players=[];
		if(Request("loadformdata"))$players=PlayersDataService::getPlayersOfTeamByPosition($this->_websoccer,$this->_db,$this->_websoccer->getUser()->getClubId($this->_websoccer,$this->_db));
...
class FacebookLoginModel extends Model {
	...
	function renderView(){ return Config("facebook_enable_login");}}
...
class FinancesSummaryModel extends Model {
	function getTemplateParameters(){
		...
		$result=$this->_db->querySelect($columns,Config('db_prefix').'_konto', 'verein_id=%d AND datum>%d GROUP BY verwendung HAVING COUNT(*)>5',array($this->_teamId,$minDate));
		$majorPositions=[];
...
class GoogleplusLoginModel extends Model {
	...
	function renderView(){ return Config("googleplus_enable_login");}}
...
class MyTeamModel extends Model {
	function getTemplateParameters(){
		...
		$players=[];
		if($teamId)$players=PlayersDataService::getPlayersOfTeamById($this->_websoccer,$this->_db,$teamId);
...
class NationalNextMatchModel extends Model {
	...
	function renderView(){
		if(!Config('nationalteams_enabled'))return FALSE;
...
class NextMatchModel extends Model {
	function getTemplateParameters(){
		if(Request("nationalteam"))$clubId=NationalteamsDataService::getNationalTeamManagedByCurrentUser($this->_websoccer,$this->_db);
		else $clubId=$this->_websoccer->getUser()->getClubId($this->_websoccer,$this->_db);
...
class NotificationsModel extends Model {
	function getTemplateParameters(){
		$user=$this->_websoccer->getUser();
		$notifications=NotificationsDataService::getLatestNotifications($this->_websoccer,$this->_db,$this->_i18n,$user->id,$user->getClubId($this->_websoccer,$this->_db),Config("notifications_max"));
		$this->_db->queryUpdate(array("seen"=>"1"),Config("db_prefix")."_notification","user_id=%d",$this->_websoccer->getUser()->id);
...
class PaypalLinkModel extends Model {
	...
	function renderView(){ return Config("paypal_enabled");}}
...
class ProfileBlockModel extends Model {
	function getTemplateParameters(){
		...
		$clubId=$user->getClubId($this->_websoccer,$this->_db);
		$team=null;
		if($clubId)$team=TeamsDataService::getTeamSummaryById($this->_websoccer,$this->_db,$clubId);
...
class ProfileModel extends Model {
	function getTemplateParameters(){
		...
		foreach($columns as$dbColumn){
			if(Request($dbColumn))$userinfo[$dbColumn]=Request($dbColumn);}
...
class StadiumEnvironmentModel extends Model {
	function getTemplateParameters(){
		...
		while($building=$result->fetch_array()){
			if($this->_i18n->hasMessage($building['name']))$building['name']=Message($building['name']);
			if($this->_i18n->hasMessage($building['description']))$building['description']=Message($building['description']);
...
class DataGeneratorService {
	...
	static function generatePlayers($websoccer,$db,$teamId,$age,$ageDeviation,$salary,$contractDuration,$strengths,$positions,$maxDeviation,$nationality=NULL){
		if(strlen($nationality))$country=$nationality;
			...
			$fromTable=Config('db_prefix').'_verein AS T INNER JOIN '.Config('db_prefix').'_liga AS L ON L.id=T.liga_id';
	...
	static function _throwException($messageKey,$parameter=null){
		$websoccer=WebSoccer::getInstance();
		$i18n=I18n::getInstance(Config('supported_languages'));
		throw new Exception(Message($messageKey,$parameter));}
	static function _createTeam($websoccer,$db,$league,$country,$cityName,$prefixes,$suffixes,$budget,$generateStadium,$stadiumNamePattern,$stadiumStands,$stadiumSeats,$stadiumStandsGrand,$stadiumSeatsGrand,$stadiumVip){
		...
		if($generateStadium){
			...
			$fromTable=Config('db_prefix').'_stadion';
	...
	static function _createPlayer($websoccer,$db,$teamId,$firstName,$lastName,$position,$mainPosition,$strengths,$country,$age,$birthday,$salary,$contractDuration,$maxDeviation){
		...
		if($teamId)$columns['verein_id']=$teamId;
			...
			$columns['transfer_ende']=$columns['transfer_start'] +Config('transfermarket_duration_days')* 24*3600;}
		$fromTable=Config('db_prefix').'_spieler';
		$db->queryInsert($columns,$fromTable);}
...
class DirectTransfersDataService {
	...
	static function executeTransferFromOffer($websoccer,$db,$offerId){
		...
		$db->queryDelete(Config("db_prefix")."_transfer_offer","player_id=%d",$offer["player_id"]);
	...
	static function _transferPlayer($websoccer,$db,$playerId,$targetClubId,$targetUserId,$currentUserId,$currentClubId,$amount,$exchangePlayer1=0,$exchangePlayer2=0){
		$db->queryUpdate(array("verein_id"=>$targetClubId,"vertrag_spiele"=>Config("transferoffers_contract_matches")),Config("db_prefix")."_spieler","id=%d",$playerId);
		$db->queryInsert(array("bid_id"=> 0,"datum"=>$websoccer->getNowAsTimestamp(),"spieler_id"=>$playerId,"seller_user_id"=>$currentUserId,"seller_club_id"=>$currentClubId,"buyer_user_id"=>$targetUserId,"buyer_club_id"=>$targetClubId,
			"directtransfer_amount"=>$amount,"directtransfer_player1"=>$exchangePlayer1,"directtransfer_player2"=>$exchangePlayer2),Config("db_prefix")."_transfer");}
	static function countReceivedOffers($websoccer,$db,$clubId){
		$columns="COUNT(*)AS hits";
		$fromTable=Config("db_prefix")."_transfer_offer";
	...
	static function _queryOffers($websoccer,$db,$startIndex,$entries_per_page,$whereCondition,$parameters){
		...
		$fromTable=Config("db_prefix")."_transfer_offer AS O INNER JOIN ".Config("db_prefix")."_spieler AS P ON P.id=O.player_id INNER JOIN ".Config("db_prefix").
			"_user AS SU ON SU.id=O.sender_user_id INNER JOIN ".Config("db_prefix")."_verein AS SC ON SC.id=O.sender_club_id INNER JOIN ".Config("db_prefix")."_verein AS RC ON RC.id=O.receiver_club_id INNER JOIN ".
			Config("db_prefix")."_user AS RU ON RU.id=RC.user_id LEFT JOIN ".Config("db_prefix")."_spieler AS EP1 ON EP1.id=O.offer_player1 LEFT JOIN ".Config("db_prefix").
...
class NotificationsDataService {
	...
	static function getLatestNotifications($websoccer,$db,$i18n,$userId,$teamId,$limit){
		...
		while($row=$result->fetch_array()){
			...
			if($i18n->hasMessage($row['message_key']))$message=Message($row['message_key']);
...
class RandomEventsDataService {
	...
	static function _createAndExecuteEvent($websoccer,$db,$userId,$clubId){
		...
		$db->queryInsert(array('user_id'=>$userId, 'team_id'=>$clubId, 'event_id'=>$randomEvent['id'],'occurrence_date'=>$websoccer->getNowAsTimestamp()),Config('db_prefix').'_randomevent_occurrence');}
	static function _executeEvent($websoccer,$db,$userId,$clubId,$event){
		...
		if($event['effect']=='money'){
			...
			if($amount)BankAccountDataService::creditAmount($websoccer,$db,$clubId,$amount,$subject,$sender);
...
class SponsorsDataService {
	static function getSponsorinfoByTeamId($websoccer,$db,$clubId){
		...
		$fromTable=Config('db_prefix').'_sponsor AS S INNER JOIN '.Config('db_prefix').'_verein AS T ON T.sponsor_id=S.id';
	...
	static function getSponsorOffers($websoccer,$db,$teamId){
		...
		$columns['S.b_sieg']='amount_win';
		$columns['S.b_meisterschaft']='amount_championship';
		$fromTable=Config('db_prefix').'_sponsor AS S';
		$whereCondition='S.liga_id=%d AND (S.min_platz=0 OR S.min_platz >= %d)'. ' AND (S.max_teams <= 0 OR S.max_teams>(SELECT COUNT(*)FROM '.Config('db_prefix').'_verein AS T WHERE T.sponsor_id=S.id AND T.sponsor_spiele>0))'
...
class TransfermarketDataService {
	...
	function extendDuration($websoccer,$db,$playerId){
		$fromTable=Config('db_prefix').'_spieler';
		$columns['transfer_ende']=$websoccer->getNowAsTimestamp()+ 24*3600*Config('transfermarket_duration_days');
		$whereCondition='id=%d';
...
class YouthPlayersDataService {
	...
	static function deleteInvalidOpenMatchRequests($websoccer,$db){
		$timeBoundary=$websoccer->getNowAsTimestamp()+Config('youth_matchrequest_accept_hours_in_advance');
...
function actionSetLanguage(){if(!isset($_POST['lang'])){global$errors;$errors[]='Please select a language.';return'printWelcomeScreen';}global$supportedLanguages;$lang=$_POST['lang'];if(key_exists($lang,$supportedLanguages)){$_SESSION['lang']=$lang;
	...
	$db->connect(Config('db_host'),Config('db_user'),Config('db_passwort'),Config('db_name'));if($db->executeQuery("SHOW TABLES LIKE '`_session`'"))return'setAdminForm';}return'printWelcomeScreen';}}
...
function printCreateUserForm($messages){?><form method='post'class='form-horizontal'><fieldset><legend><?php echo$messages['user_formtitle']?></legend><div class='control-group'><label class='control-label'for='name'><?php echo$messages['label_name']?></label>
	...
	<?php echo$messages['label_password']?></label><div class='controls'><input type='password'id='password'name='password'required value='<?php echo isset(escapeOutput($_POST['password'])?escapeOutput($_POST['password']):'');?>'></div></div><br>
	<div class='control-group'><label class='control-label'for='email'><?php echo$messages['label_email']?></label><div class='controls'><input type='email'id='email'name='email'required value='<?php echo (isset(escapeOutput($_POST['email']))?
...
function setAdminForm($messages){?><form method='post'class='form-horizontal'><fieldset><legend><?php echo$messages['user_formtitle']?></legend><div class='control-group'><label class='control-label'for='db_host'><?php echo$messages['label_db_host']?></label>
	...
	<label class='control-label'for='password'><?php echo$messages['label_password']?></label><div class='controls'><input type='password'id='password'name='password'required value='<?php echo isset(escapeOutput($_POST['password'])?escapeOutput($_POST['password']):'');
