...
function actionSetLanguage(){if(!isset($_POST['lang'])){global$errors;$errors[]='Please select a language.';return'printWelcomeScreen';}global$supportedLanguages;$lang=$_POST['lang'];if(key_exists($lang,$supportedLanguages)){$_SESSION['lang']=$lang;
	if($setAdmin=1)return'setAdminForm';else return'printSystemCheck';}return'printWelcomeScreen';}
...
function is__writable($path){if($path[strlen($path)-1]=='/')return is__writable($path.uniqid(mt_rand()).'.tmp');elseif(is_dir($path))return is__writable($path.'/'.uniqid(mt_rand()).'.tmp');$rm=file_exists($path);$f=@fopen($path,'a');if($f===false)return false;fclose($f);if(!$rm)unlink($path);return true;}
...
function setAdminForm($messages){?><form method='post'class='form-horizontal'><fieldset><legend><?php echo$messages['user_formtitle']?></legend><div class='control-group'><label class='control-label'for='db_host'><?php echo$messages['label_db_host']?></label>
	<div class='controls'><input type='text'id='db_host'name='db_host'required value="<?php echo(isset($_POST['db_host']))?$_POST['db_host']:'localhost';?>"><span class='help-inline'><?php echo$messages['label_db_host_help']?></span></div></div><div class='control-group'>
	<label class='control-label'for='db_name'><?php echo$messages['label_db_name']?></label><div class='controls'><input type='text'id='db_name'name='db_name'required value="<?php echo(isset($_POST['db_name']))?$_POST['db_name']:'';?>"></div></div>
