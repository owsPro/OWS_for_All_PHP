...
class FormBuilder {
	...
	static function createForeignKeyField($i18n,$fieldId,$fieldInfo,$fieldValue){
		...
		if($items['hits'] <= 20){
			echo '<select id=\''.$fieldId.'\' name=\''.$fieldId.'\'>';
			echo '<option value=\'\'>'.Message(escapeOutput('manage_select_placeholder')).'</option>';
			...
			while($row=$result->fetch_array()){
				...
				echo '>'. escapeOutput($label).'</option>';}
			echo '</select>';}
		else echo '<input type=\'hidden\' class=\'pkpicker\' id=\''.$fieldId.'\' name=\''.$fieldId.'\' value=\''.$fieldValue.'\' data-dbtable=\''.escapeOutput($fieldInfo['jointable']).'\' data-labelcolumns=\''.escapeOutput($fieldInfo['labelcolumns']).'\' data-placeholder=\'' .
				Message('manage_select_placeholder').'\'>';
		echo ' <a href=\'?site=manage&entity='.escapeOutput($fieldInfo['entity']).'&show=add\' title=\''.Message('manage_add').'\'><i class=\'icon-plus-sign\'></i></a>';}}
class FrontMessage {
...
class BorrowPlayerController extends Controller {
	function executeAction($parameters){
		...
		if($player["lending_owner_id"]>0)throw new Exception(Message("lending_hire_err_borrowed_player"));
		if($player["lending_fee"]==0)throw new Exception(Message("lending_hire_err_notoffered"));
		if($player["player_transfermarket"]>0)throw new Exception(Message("lending_err_on_transfermarket"));
...
class ChooseTeamController extends Controller {
	function executeAction($parameters){
		...
		if($user->getClubId($this->_websoccer,$this->_db)>0)throw new Exception(Message("freeclubs_msg_error_user_is_already_manager"));
		$teamId=$parameters["teamId"];
		$fromTable=Config("db_prefix")."_verein";
...
class LendPlayerController extends Controller {
	function executeAction($parameters){
		...
		if($player["lending_owner_id"]>0)throw new Exception(Message("lending_err_borrowed_player"));
		if($player["lending_fee"]>0)throw new Exception(Message("lending_err_alreadyoffered"));
		if($player["player_transfermarket"]>0)throw new Exception(Message("lending_err_on_transfermarket"));
		$teamSize=TeamsDataService::getTeamSize($this->_websoccer,$this->_db,$clubId);
...
class MicropaymentRedirectController extends Controller {
	function executeAction($parameters){
		...
		$queryStr .= '&seal='.$seal;
		$paymentUrl .= $queryStr;
		escapeOutput(header('Location: '.$paymentUrl));
		exit;
		return null;}}
...
class SaveFormationController extends Controller {
	...
	function validatePlayer($playerId,$players,$bench=FALSE){
		...
		if($players[$playerId]['matches_injured']>0||$players[$playerId]['matches_blocked']>0)throw new Exception(Message('formation_err_blockedplayer'));
...
function printCreateUserForm($messages){?><form method='post'class='form-horizontal'><fieldset><legend><?php echo$messages['user_formtitle']?></legend><div class='control-group'><label class='control-label'for='name'><?php echo$messages['label_name']?></label>
	...
	<div class='controls'><input type='password'id='password'name='password'required value='<?php echo escapeOutput(isset($_POST['password'])?$_POST['password']:'');?>'</div></div><br>
...
function setAdminForm($messages){?><form method='post'class='form-horizontal'><fieldset><legend><?php echo$messages['user_formtitle']?></legend><div class='control-group'><label class='control-label'for='db_host'><?php echo$messages['label_db_host']?></label>
	...
	for='password'><?php echo$messages['label_password']?></label><div class='controls'><input type='password'id='password'name='password'required value="<?php echo escapeOutput(isset($_POST['password'])?$_POST['password']:'');?>"></div></div><div class='control-group'>
	<label class='control-label'for='email'><?php echo$messages['label_email']?></label><div class='controls'><input type='email'id='email'name='email'required value="<?php echo(escapeOutput(isset($_POST['email'])))?escapeOutput($_POST['email']):'';?>"></div></div>
	</fieldset><div class='form-actions'><button type='submit'class='btn btn-primary'><?php echo$messages['button_next'];?></button></div><input type='hidden'name='action'value='actionSaveUser'></form><?php }
